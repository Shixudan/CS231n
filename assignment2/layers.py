...
def batchnorm_backward(dout, cache):
    ...
    dx_hat = dout * gamma
    dmu1 = dx_hat / mu2
    dmu2 = - np.sum(mu1 / mu2 ** 2 * dx_hat, axis=0)
    dx1 = dmu1
    dmean1 = - np.mean(dmu1, axis=0)

    dvar = 0.5 * (var + eps) ** (-0.5) * dmu2
    dx_square = 1.0 / N * dvar
    dmu3 = 2.0 * (x - mean) * dx_square
    dx2 = dmu3
    dmean2 = -np.sum(dmu3, axis=0)
...
def layernorm_backward(dout, cache):
    ...
    dx_hat = gamma * dout
    dvar = np.sum((x - mean) * dx_hat, axis=1, keepdims=True) * (-0.5 / np.sqrt(var + eps) ** 3)
    dmean = -1 / np.sqrt(var + eps) * np.sum(dx_hat, axis=1, keepdims=True) + \
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        mask = (np.random.randn(*x.shape) < p) / p # *x.shape 与 x.shape[0] 的结果是一致的
        out = x * mask
        pass
...
def max_pool_forward_naive(x, pool_param):
    ...
    for j in range(H_out):
        for k in range(W_out):
            out[:, :, j, k] = np.max(x[:, :, j * stride: pool_height + j * stride, k * stride: pool_width + k * stride], axis=(2, 3))
    pass
...
def max_pool_backward_naive(dout, cache):
    ...
    for j in range(H_out):
        for k in range(W_out):
            x_masked = x[:, :, j * stride: pool_height + j * stride, k * stride: pool_width + k * stride]
            max_x_masked = np.max(x_masked, axis=(2, 3))[:, :, None, None]
            dx[:, :, j * stride: j * stride + pool_height, k * stride: k * stride + pool_width] += (x_masked == max_x_masked) * dout[:, :, j, k][:, :, None, None]
    pass
